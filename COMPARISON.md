...
|---|---|---|
| Branchcut | release, Rust 1.96.0 | Subject under test |
...
| Case | Branchcut | fast-glob | tinyglobby | Branchcut == both |
|---|---:|---:|---:|---|
| Literal extension | 1 | 1 | 1 | PASS |
| Single wildcard | 5,000 | 5,000 | 5,000 | PASS |
| Question wildcard | 10 | 10 | 10 | PASS |
| Character class | 10 | 10 | 10 | PASS |
| Globstar | 5,000 | 5,000 | 5,000 | PASS |
| Brace alternatives | 10,000 | 10,000 | 10,000 | PASS |
| Multiple positive patterns | 500 | 500 | 500 | PASS |
| Explicit exclusions | 10,000 | 10,000 | 10,000 | PASS |
...
- zlob used a temporary Zig harness calling its public `zlob.match` filesystem API directly with `std.heap.c_allocator`, 3 warmups, and 10 measured iterations in one process.
- The harness used `nosort=true`, matching Branchcut's count-only streaming path.
- The official zlob C-compatible benchmark function was also attempted. On Windows it returned zero matches for the drive-letter path, while the official zlob tree-size benchmark is disabled on Windows because it requires libc glob. It is excluded from the numeric comparison rather than treated as a valid zero-time result.
...
|---|---:|---:|---:|
| Branchcut release traversal | **18.393 ms** | 22.69 ms | 13,000 |
| tinyglobby 0.2.14 | 27.227 ms | 30.681 ms | 13,000 |
| fast-glob 3.3.3 | 28.721 ms | 34.115 ms | 13,000 |
| zlob v1.6.3 public `match` API | 120.090 ms average | not captured | 13,000 |
...
These are workload-specific results. zlob's separate parallel walker was not used in this comparison; it may have a different performance profile.
...
| Capability | Branchcut | fast-glob 3.3.3 | tinyglobby 0.2.14 | zlob tested CLI |
|---|---|---|---|---|
| Recursive globstar | Supported | Supported | Supported | Basic cases supported; nested case above mismatched |
| Braces | Flat alternatives | Supported | Supported | Supported |
| Character classes | Supported | Supported | Supported | Supported |
| Multiple positives | One shared traversal program | Supported | Supported | CLI accepts one pattern per invocation |
| Explicit ignore/exclude | Supported; subtree pruning | Supported via `ignore` | Supported via `ignore` | CLI tested had no equivalent exclude option |
| `.gitignore` | Not implemented | Not by default | Not by default | Supported by zlob flags/API |
| Hidden default | Excluded | Excluded | Excluded | Excluded |
...
| Global sorting | Optional | Configurable | Configurable | `-s` |
| Metadata predicates | Not implemented | Limited option surface | Limited option surface | Walker metadata API |
| Hierarchical gitignore | Not implemented | Not claimed as core default | Not claimed as core default | Supported |
| Extglob | Not implemented | Supported subset | Supported through underlying engine | Supported |
| JSON output | Not implemented | No built-in JSON output | No built-in JSON output | No built-in JSON output |
| External runtime deps | None | 17 transitive npm deps | 2 transitive npm deps | Zig/runtime build ecosystem |
